...
LSB替换是一种著名的隐写方法。在这种嵌入模式中，只有载体的LSB平面依据伪随机数生成器被秘密比密文比特流重写。因此，一些结构性的对称（在隐藏数据时）从不较少偶数像素和增加奇数像素被引入，致使在低嵌入率的情况下，即使使用一些公开算法，比如$\chi ^2$攻击[2]，R/S块分析[3]，样本对分析[4]，和广义上的结构性隐写分析框架[5]，[6],也很容易探查到隐藏消息的存在。
