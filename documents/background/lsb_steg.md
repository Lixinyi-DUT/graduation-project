...
针对基于LSB的图像隐写分析的方法主要有$\chi^2$测验[@westfeld1999attacks]，样本对分析[@dumitrescu2002steganalysis]和RS隐写分析[@fridrich2001reliable]。

### $\chi^2$测验
$\chi^2$测验，也就是值对（PoV）攻击，是统计隐写分析的先驱。$\chi^2$攻击的设计基于图像直方图来检测LSB嵌入。

为了理解值对我们可以先观察载体和使用LSB满容量嵌入得到的伪装的直方图，直方图统计了每个像素值
